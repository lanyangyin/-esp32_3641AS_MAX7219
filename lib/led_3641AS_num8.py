...
def display_line(strs):
    if type(strs) == int or float:
        strs = str(strs)
    if has_element_not_in_list2(list(strs), list(segments_set | {'.'})) or '..' in strs or 0 > len(
            strs.replace('.', '')) or 8 < len(strs.replace('.', '')) or (strs.startswith('.') and strs.count(".") != 1):
